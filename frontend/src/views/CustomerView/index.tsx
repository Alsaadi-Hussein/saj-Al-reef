...
const T = {
  ar: {
    restaurant: 'ساج الريف | طاولة 5',
    qrSub: 'رمز QR · المسح المباشر 1',
  ...
  en: {
    restaurant: 'SAJ ALREEF | Table 5',
    qrSub: 'QR Code · Direct Scan 1',
        ...
        <div className="px-4 pt-4 pb-3 border-b border-c3 flex-shrink-0" style={{ background: '#111111' }}>
          <div className="flex justify-between items-start">
            ...
            <div className="text-right">
              <div className="text-[14px] font-semibold text-gold">{t.restaurant}</div>
              <div className="text-[10px] text-white/40 mt-0.5">{t.qrSub}</div>
            </div>
          </div>
        </div>
...
// ─── Home Screen ─────────────────────────────────────────────
function HomeScreen({ onNav, lang, t }: { onNav: (tab: PhoneTab) => void; lang: Lang; t: typeof T['ar'] }) {
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel('customer-order-tracking')
  ...
  async function doQuickCall(type: string) {
    await api.sendAlert(TABLE, type, type === 'نادل' || type === 'Waiter' ? '🧑‍🍳' : type === 'ماء' || type === 'Water' ? '💧' : '🧻')
  }

  async function submitRating() {
    setRatingLoading(true)
    await api.submitRating(ratingFood, ratingService, ratingOverall, comment, TABLE_NUM)
    setRatingDone(true)
      ...
      <div className="grid grid-cols-4 gap-1.5">
        {t.quickCalls.map((item, i) => (
          <button
            ...
            className="bg-c2 border border-c3 rounded-[9px] py-2 text-[11px] text-white/70 hover:border-gold/40 hover:text-gold transition-all cursor-pointer"
          >
            {item}
          </button>
        ))}
...
// ─── QR Screen ───────────────────────────────────────────────
function QrScreen({ lang }: { lang: Lang }) {
  return (
    <div className="flex flex-col items-center justify-center h-full p-6 gap-4">
      ...
      <div className="text-[11px] text-white/30 text-center">{lang === 'ar' ? 'ساج الريف | طاولة 5' : 'SAJ ALREEF | Table 5'}</div>
    </div>
  )
}
